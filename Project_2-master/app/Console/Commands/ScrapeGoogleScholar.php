...
class ScrapeGoogleScholar extends Command
{
    ...
    public function handle()
    {
        ...
        $this->info("Scholar Name: " . $data['scholar_Name']);
        $this->info("Interests: " . implode(', ', $data['intersts']));
        $this->info("Publications:");
        foreach ($data['publications'] as $publication) {
            $this->info("- {$publication['title']} ({$publication['year']}), Citations: {$publication['citations']}");
        }
        return 0;
    }
}
